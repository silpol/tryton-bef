...
class GisThreadedConnectionPool(ThreadedConnectionPool):
    """Customize connection creation: enable gis support"""
    def _connect(self, key=None):
        conn = super(GisThreadedConnectionPool, self)._connect(key)
        try:
            dbgis.register_psycopg(conn, conn)
        except Exception:
            pass
        return conn
