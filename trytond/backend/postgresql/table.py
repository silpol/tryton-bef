...
class TableHandler(TableHandlerInterface):
    ...
    def add_geometry_column(self, column_name, column_type, srid, dimension):
        if self.column_exist(column_name):
            # Check for type and eventual migration
            return
        self.cursor.execute('SELECT AddGeometryColumn(%s, %s, %s, %s, %s)',
            (self.table_name, column_name, srid, column_type[0], dimension))
        self._update_definitions()
    ...
    def index_action(self, column_name, action='add', table=None):
        ...
        if action == 'add':
            if test_index_name in self._indexes:
                return
            use_gist = any(self._columns[col]['typname'] == 'geometry'
                for col in column_name)
            index_type = "USING GIST" if use_gist else ""
            self.cursor.execute('CREATE INDEX "' + index_name + '" '
                'ON "' + self.table_name + '" %s ( ' % index_type
