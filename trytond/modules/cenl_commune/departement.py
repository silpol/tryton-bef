...
class Departement(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('cenl.report_departement_edit')
...
class DepartementQGis(QGis):
    __name__ = 'cenl.departement.qgis'
    TITLES = {'cenl.departement': u'Département'}

class GenerateD(Wizard):
    __name__ = 'cenl.departement_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        departement = Pool().get('cenl.departement')
