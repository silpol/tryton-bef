...
class Commune(Mapable, ModelSQL, ModelView):
    u'Commune Française'
    __name__ = 'cenl.commune'

    _order = [('nom', 'ASC'), ('postal', 'ASC')]
    ...
    @classmethod
    @ModelView.button_action('cenl.report_commune_edit')
...
class CommuneQGis(QGis):
    __name__ = 'cenl.commune.qgis'
    TITLES = {'cenl.commune': u'Commune'}

class GenerateC(Wizard):
    __name__ = 'cenl.commune_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        commune = Pool().get('cenl.commune')
