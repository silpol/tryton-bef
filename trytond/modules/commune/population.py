...
class Population(ModelSQL, ModelView):
    ...
    full_population = fields.Function(fields.Text('Full Population'),
            'get_full_population')

    com = fields.Many2One('commune.commune', ondelete='CASCADE',
            string='com', select=True)

    def get_full_population(self, name):
        return ['\n'.join(x for x in (self.annee, self.total, self.pop_0014,
            self.pop_1529, self.pop_3044, self.pop_4559, self.pop_6074,
            self.pop_75p))]
