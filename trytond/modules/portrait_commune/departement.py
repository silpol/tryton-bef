...
class Departement(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('portrait.report_departement_edit')
...
class DepartementQGis(QGis):
    __name__ = 'portrait.departement.qgis'
    TITLES = {'portrait.departement': u'Département'}

class GenerateD(Wizard):
    __name__ = 'portrait.departement_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        departement = Pool().get('portrait.departement')
