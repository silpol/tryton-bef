...
class Region(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('portrait.report_region_edit')
...
class RegionQGis(QGis):
    __name__ = 'portrait.region.qgis'
    TITLES = {'portrait.region': u'Région'}

class GenerateR(Wizard):
    __name__ = 'portrait.region_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        region = Pool().get('portrait.region')
