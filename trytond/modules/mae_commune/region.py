...
class Region(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('mae.report_region_edit')
...
class RegionQGis(QGis):
    __name__ = 'mae.region.qgis'
    TITLES = {'mae.region': u'Région'}

class GenerateR(Wizard):
    __name__ = 'mae.region_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        region = Pool().get('mae.region')
