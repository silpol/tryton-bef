...
class Departement(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('mae.report_departement_edit')
...
class DepartementQGis(QGis):
    __name__ = 'mae.departement.qgis'
    TITLES = {'mae.departement': u'Département'}

class GenerateD(Wizard):
    __name__ = 'mae.departement_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        departement = Pool().get('mae.departement')
