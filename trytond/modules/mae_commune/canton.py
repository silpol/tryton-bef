...
class Canton(Mapable, ModelSQL, ModelView):
    ...
    @classmethod
    @ModelView.button_action('mae.report_canton_edit')
...
class CantonQGis(QGis):
    __name__ = 'mae.canton.qgis'
    TITLES = {'mae.canton': u'Canton'}

class GenerateCa(Wizard):
    __name__ = 'mae.canton_generate'

    @classmethod
    def execute(cls, session, data, state_name):
        canton = Pool().get('mae.canton')
